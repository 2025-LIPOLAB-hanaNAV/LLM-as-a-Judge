...
"proper_masking": "짧은 이유(≤20자)"
    ...
    def evaluate_single_question(self, question_data: Dict[str, str]) -> Dict[str, Any]:
        ...
        logger.info(f"PII 평가 시작: {question_id}")
        ...
        logger.info(f"PII 평가 결과: {evaluation_result[:100]}..." if evaluation_result else "평가 결과 없음")
        ...
        try:
            for i, question_data in enumerate(self.test_data):
                ...
                try:
                    ...
                    logger.error(f"질문 {question_data['question_id']} PII 평가 실패: {e}")
                    # 실패한 경우에도 기본 구조 유지
                    results.append({
                        "question_id": question_data["question_id"],
                        "no_unnecessary_pii": 0,
    ...
    def save_results(self, results: List[Dict[str, Any]], output_path: str = "rag_evaluation_results_pii.json"):
        """PII 평가 결과 저장"""
        try:
            with open(output_path, 'w', encoding='utf-8') as f:
                json.dump(results, f, ensure_ascii=False, indent=2)
            logger.info(f"PII 평가 결과 저장 완료: {output_path}")
        except Exception as e:
            logger.error(f"PII 평가 결과 저장 실패: {e}")
    
    def print_summary(self, results: List[Dict[str, Any]]):
        """PII 평가 결과 요약 출력"""
        if not results:
            print("PII 평가 결과가 없습니다.")
            return
...
def main():
    """메인 실행 함수"""
    # 설정
    test_dataset_path = "./test_dataset.json"  # TODO: 위치 경로 변경하기
    ollama_url = "http://localhost:11435"
    
    # PII 평가 시스템 초기화
    evaluator = RAGEvaluationSystemPII(test_dataset_path, ollama_url)
    
    # 모든 질문 PII 평가
    print("RAG 기반 챗봇 PII 평가 시작...")
    results = evaluator.evaluate_all_questions()
    
    # 결과 저장
    evaluator.save_results(results, "rag_evaluation_results_pii.json")
